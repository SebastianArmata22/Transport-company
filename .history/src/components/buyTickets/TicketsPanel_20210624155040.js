...
const TicketsPanel = ({tickets, user, setShowShopping,setUserData}) => {
    ...
    const discount=()=>{
        if(points<user.points && parseFloat(tickets.price)-parseFloat(price)<=points/10){
            setPrice(prevState=>prevState-points/10)
        }
    }
    ...
    return (
        ...
        <h4>Rabat</h4>
            <div className={classes.infoPanel}>
             <p>1 punkt = 0,10zł, możesz wykorzystać max {user.points}pkt.</p>
            </div>
             <input  type="text" pattern="[0-9]*" value={points} onChange={changePoints} placeholder="Wpisz ilość punktów, 1 punkt = 0,10zł"></input>
             <br></br>
