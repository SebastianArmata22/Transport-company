...
const TicketsPanel = ({tickets, user, setShowShopping,setUserData}) => {
    ...
    const discount=async ()=>{
        if((parseFloat(tickets.price)-parseFloat(price))===0){
            if(user.points>0){
                setPrice(prevState=>SMath.round((prevState-user.points/10)*100)/100<0 ? 0: Math.round((prevState-user.points/10)*100)/100)
                ...
                NotificationManager.error('Brak punktów')
            }
        }
        else{
            NotificationManager.error('Wykorzystano już punkty')
        }
    ...
    const buyTicket=async()=>{
        if(blik!==""){
            await database.collection('tickets').doc(uid).collection('ticket').add({
                ...
                await database.collection("users").doc(uid).update({
                    points: (parseFloat(tickets.price)-parseFloat(price))===0? Math.round(tickets.price)+ userData.points: Math.round(tickets.price)
                })
                setUserData(prevState => ({
                    ...prevState,
                    points:(parseFloat(tickets.price)-parseFloat(price))===0? Math.round(tickets.price)+ userData.points: Math.round(tickets.price)
